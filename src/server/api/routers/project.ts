...
export const projectRouter = createTRPCRouter({
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const old_contract_award = input.contract_award_id
        ? await prisma.contract_award.findUnique({
      ...
      if (
        old_project &&
        old_project.project_name !== updated_project.project_name
      )
        emailContent += `Project Name: ${
          old_project.project_name || "N/A"
        } ---> ${updated_project.project_name || "N/A"} \n`;

      if (
        old_project &&
        old_contract_award !== undefined &&
        old_contract_award !== null &&
        old_contract_award.contract_num !== input.contract_num
      ) {
        emailContent += `Contract Number: ${
          old_contract_award.contract_num || "N/A"
        } ---> ${input.contract_num || "N/A"} \n`;
      }

      if (old_project && old_project.ccar_num !== updated_project.ccar_num)
        emailContent += `CCAR Number: ${old_project.ccar_num || "N/A"} ---> ${
          updated_project.ccar_num || "N/A"
        } \n`;

      if (
        old_project &&
        old_project.contractor_id !== updated_project.contractor_id
      ) {
        ...
        const oldContractorName = oldContractor
          ? oldContractor.contractor_name
          : "None";
        ...
        const newContractorName = newContractor
          ? newContractor.contractor_name
          : "None";

        emailContent += `Contractor: ${oldContractorName || "N/A"} ---> ${
          newContractorName || "N/A"
        } \n`;
      }
      ...
      if (
        old_project &&
        old_project.requirement_type_id !== updated_project.requirement_type_id
      ) {
        ...
        const oldReqTypeName = oldReqType
          ? oldReqType.requirement_type
          : "None";
        ...
        const newReqTypeName = newReqType
          ? newReqType.requirement_type
          : "None";

        emailContent += `Requirement Type: ${oldReqTypeName || "N/A"} ---> ${
          newReqTypeName || "N/A"
        } \n`;
      }

      if (old_project && old_project.summary !== updated_project.summary)
        emailContent += `Capability Summary: ${
          old_project.summary || "N/A"
        } ---> ${updated_project.summary || "N/A"} \n`;
      ...
      if (emailContent) {
        for (const iptMember of iptMembersUsers) {
          ...
          await sendEmail(
            ...
            `METIS - ${input.project_name || "N/A"} has been updated by ${
              user.user_name || "N/A"
            } (${user.user_email || "N/A"})`,
            `${user.user_name || "N/A"} (${
              user.user_email || "N/A"
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (const iptMember of iptMembersUsers) {
        ...
        await sendEmail(
          iptMember.user_email,
          `METIS - ${input.id || "N/A"} has been deleted by ${
            user.user_name || "N/A"
          } (${user.user_email || "N/A"})`,
          `${user.user_name || "N/A"} (${
            user.user_email || "N/A"
          }) deleted the following project in METIS: \n\n ${input.id || "N/A"} `
        );
      }
  ...
  search: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (user.user_role === "Admin" || user.user_role === "IPT_Member") {
        return input.filterQuery && input.filterType === "project_name"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
                  WHERE project_name LIKE ${"%" + input.filterQuery + "%"}`
          : input.filterQuery && input.filterType === "contract_number"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
                  WHERE contract_num LIKE ${"%" + input.filterQuery + "%"}`
          : input.filterQuery && input.filterType === "contract_value"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
                  WHERE contract_value LIKE ${"%" + input.filterQuery + "%"}`
          : input.filterQuery && input.filterType === "contractor"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
                  WHERE contractor_name LIKE ${"%" + input.filterQuery + "%"}`
          : input.filterQuery && input.filterType === "contract_status"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
                  WHERE contract_status = ${input.filterQuery}`
          : input.filterQuery && input.filterType === "dependency_status"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
                  WHERE dependency_status = ${input.filterQuery}`
          : input.filterQuery && input.filterType === "financial_status"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
                  WHERE financial_status = ${input.filterQuery}`
          : input.filterQuery && input.filterType === "schedule_status"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
                  WHERE schedule_status = ${input.filterQuery}`
          : input.filterQuery && input.filterType === "branch"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
        ...
        return input.filterQuery && input.filterType === "project_name"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
                  WHERE project_name LIKE ${
                    "%" + input.filterQuery + "%"
                  } AND contractor_id = ${current_id}`
          : input.filterQuery && input.filterType === "contract_number"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
                  WHERE contract_num LIKE ${
                    "%" + input.filterQuery + "%"
                  } AND contractor_id = ${current_id}`
          : input.filterQuery && input.filterType === "contract_value"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
                  WHERE contract_value LIKE ${
                    "%" + input.filterQuery + "%"
                  } AND contractor_id = ${current_id}`
          : input.filterQuery && input.filterType === "contractor"
          ? await prisma.$queryRaw<view_project[]>`SELECT * FROM view_project
