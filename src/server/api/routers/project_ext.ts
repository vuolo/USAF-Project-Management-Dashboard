...
export const project_ext = {
  getProjectsWithUpcomingDueMilestones: protectedProcedure
    .input(
      z.object({
      ...
      await Promise.all(
        projects.map(async (project) => {
          if (isAdmin && input.allProjects)
            final.push({
              ...project,
              project_milestones: project.project_milestones.filter(
                (milestone) => {
                  if (!milestone.end_date) return false;
                  return (
                    milestone.end_date >= currentDate &&
                    milestone.end_date <= nextDate
                  );
                }
              ),
            });
          const iptMembers = await getIPTMembers(project.id);
          const isMember = iptMembers.find((m) => m.id === user.id)
            ? true
            : false;
          if (isMember) {
            final.push({
